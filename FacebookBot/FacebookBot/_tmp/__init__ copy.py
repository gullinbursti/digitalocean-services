...
def send_request_queue(rs):
  ...
  # 
  # try:
  #   _response = urllib2.urlopen("http://beta.modd.live/api/bot_tracker.php?category=%s&action=%s&label=%s" % (str(category), str(action), str(label)), timeout=3)
  #  
...
def start_chat(sender_id):
  session_obj = get_help_session(sender_id)
...
def buildQueue(data):
  logger.info("buildQueue(data={data}".format(data=data))
  ...
  req = [grequests.post("https://graph.facebook.com/v2.6/me/messages"
                      , params=params, headers=headers, data=datum)
                      for datum in data]
  return (req)
        ...
        if messaging_event.get('message'):
          ...
          #-- query for topic
            ...
            if quick_reply is None:
              ...
              help_session = set_help_session({
                ...
                'messages': [quick_reply.split("_")[-1]]
              })
          ...
          #-- requesting help
            ...
            help_session = set_help_session({
              ...
              'ended': help_session['ended'],
              'topic': help_session['topic'],
              'messages': help_session['messages']
            })
                    
                    
          #-- in session
          elif help_state == 3:
            logger.info("----------=IN ACTIVE SESION SESSION ({ignored}/{total})=----------".format(ignored=help_session['ignore_count'], total=))
            
            if help_session['ignore_count'] >= Const.MAX_IGNORES - 1:
              logger.info("-=- TOO MANY UNREPLIED - ({count}) CLOSE OUT SESSION @({timestamp}) -=-".format(count=Const.MAX_IGNORES, timestamp=time.strftime("%Y-%m-%d %H:%M:%S")))
            ...
            #-- continue convo
              ...
              help_session = set_help_session({
                ...
                'ended': help_session['ended'],
                'topic': help_session['topic'],
                'messages': help_session['messages'].append(message_text)
              })
          ...
          #-- faq pending
            ...
            #-- prompt y/n
            if not quick_reply:
              logger.info("----------=PROMPT FOR FAQ ({topic})=----------".format(topic=help_session['topic']))
              ...
              help_session = set_help_session({
                ...
                'ended': help_session['ended'],
                'topic': help_session['topic'],
                'messages': help_session['messages'].append(quick_reply.split("_")[-1])
              })
              
              
              if quick_reply.split("_")[-1] == "yes":
                for entry in fetch_faq(help_session['topic']):
