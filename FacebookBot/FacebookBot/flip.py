...
def fetch(profile_id=76561198277603515):
    ...
    for asset in response.json()['descriptions']:
        ...
        inventory[key] = {
            ...
            'description': "",  # if asset['descriptions'] is None or len(asset['descriptions']) < 3 else asset['descriptions'][2]['value'].encode('utf-8'),
    ...
    try:
        with conn:
            ...
            for key, value in inventory.iteritems():
                ...
                cur.execute('INSERT INTO `flip_items` (`id`, `type_id`, `asset_name`, `description`, `game_name`, `image_url`, `icon_url`, `quantity`, `price`, `enabled`, `updated`, `added`) SELECT NULL, `type`, `name`, `description`, `game_name`, `image_url`, `icon_url`, `quantity`, `min_sell`, `enabled`, UTC_TIMESTAMP(), UTC_TIMESTAMP  FROM `flip_inventory` WHERE `quantity` > 0 GROUP BY `name` ORDER BY `added`;')
                conn.commit()
