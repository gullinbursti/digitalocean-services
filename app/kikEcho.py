...
def delayed_kik_send(messages, delay=8):
  print "%d\tdelayed_kik_send(messages=%s, delay=%s)" % (int(time.time()), messages, delay)
          ...
          # -=-=-=-=-=-=-=-=-=- HELP CONNECT CON't -=-=-=-=-=-=-=-
              ...
              kik.send_messages([
                TextMessage(
                  to = message.from_user,
                  chat_id = message.chat_id,
                  body = "Top %s players found. Your question has been added to the %s queue and will be answered shortly." % (gameHelpList[message.from_user], gameHelpList[message.from_user]),
                  type_time = 1500,
                ),
                TextMessage(
                  to = message.from_user,
                  chat_id = message.chat_id,
                  body = "Pro tip: you can still keep asking questions, each will be added to the %s queue." % (gameHelpList[message.from_user]),
                  type_time = 1500,
                ),
              ])
