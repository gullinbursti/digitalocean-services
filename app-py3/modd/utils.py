...
def send_evt_tracker(category="", action="", label="", value=0):
  ...
  urls = [
    ...
    "http://beta.modd.live/api/bot_tracker.php?src=kik&category=user-message&action=user-message&label={label}&value={value}&cid={cid}".format(label=label, value=value, cid=hashlib.md5(label.encode()).hexdigest())
  ]
