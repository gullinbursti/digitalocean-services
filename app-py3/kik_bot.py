...
Const.ALWAYS_WIN_PATTERN = 'beccaplt3'
...
def cancel_keyboard(hidden=False):
    keyboard = [
        SuggestedResponseKeyboard(
            hidden = hidden,
            responses = [
...
def default_content_reply(message, topic, level):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute('SELECT `id`, `youtube_id`, `video_title`, `image_url`, `video_url`, `topic_name` FROM `topic_content` WHERE `topic_name` = %s AND `level` = %s ORDER BY RAND() LIMIT 1;', (topic, level))
...
def flip_item_message(message):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute('SELECT `fb_id` FROM `item_winners` WHERE `kik_name` = %s LIMIT 1;', (message.from_user,))
            if cur.fetchone() is not None:
                win_boost = 0.5

            cur.execute('SELECT COUNT(*) AS `tot` FROM `item_winners` WHERE `kik_name` = %s AND `added` > DATE_SUB(NOW(), INTERVAL 2 HOUR);', (message.from_user,))
            row = cur.fetchone()
            if row is not None:
                total_wins = row['tot']
            ...
            if row is not None:
                ...
                item_flips[message.chat_id]['flip'] = random.uniform(0, win_boost) <= ((1 / float(4)) * (abs(1 - (total_wins * 0.01)))) or re.search(Const.ALWAYS_WIN_PATTERN, message.from_user, re.IGNORECASE) is not None
...
def flip_timer(message):
    ...
    if message.chat_id not in item_flips:
        return
...
def flip_result(message):
    ...
    if item_flips[message.chat_id]['flip'] is True or message.from_user == "beccaplt3":
        ...
        item_flips[message.chat_id]['pin'] = pin_code
        ...
        try:
            with conn.cursor() as cur:
                ...
                if message.from_user != "beccaplt3":
                    cur.execute('UPDATE `flip_inventory` SET `quantity` = `quantity` - 1 WHERE `id` = %s AND `quantity` > 0 LIMIT 1;', (item_flips[message.chat_id]['id'],))
                ...
                item_flips[message.chat_id]['claim_id'] = cur.fetchone()['id']
                cur.close()

                item_flips[message.chat_id]['trade_url'] = "_{PENDING}_"
                ...
                try:
                    kik.send_messages([
                        ...
                        TextMessage(
                            to = message.from_user,
                            chat_id = message.chat_id,
                            body = "WINNER! You won {item_name} for {game_name}.\n\n1. Enter your Steam Trade URL\n\n2. Follow instructions: {claim_url}\n\n3. Wait 6 hours.".format(item_name=item_flips[message.chat_id]['name'], game_name=item_flips[message.chat_id]['game_name'], claim_url="taps.io/gamebotsc"),
                            type_time = 250
                        )
                    ])
                except KikError as err:
        ...
        for message in messages:
            ...
            # -=-=-=-=-=-=-=-=- DELIVERY RECEIPT MESSAGE -=-=-=-=-=-=-=-=-
                ...
                try:
                    conn = sqlite3.connect("{script_path}/data/sqlite3/kikbot.db".format(script_path=os.path.dirname(os.path.abspath(__file__))))
                    cur = conn.cursor()
                    cur.execute('SELECT id FROM mentions WHERE participant = ? AND enabled = 1 ORDER BY added DESC LIMIT 1;', (message.from_user,))

                    row = cur.fetchone()
                    if row is not None:
                        try:
                            cur.execute('UPDATE mentions SET enabled = 0 WHERE id = ? LIMIT 1;', (row[0],))
                            conn.commit()
            ...
            # -=-=-=-=-=-=-=-=- START CHATTING -=-=-=-=-=-=-=-=-
                ...
                try:
                    with conn.cursor() as cur:
                        cur.execute('INSERT INTO `kikbot_logs` (`id`, `username`, `chat_id`, `body`, `added`) VALUES (NULL, %s, %s, %s, NOW());', (message.from_user, message.chat_id, "__{START-CHATTING}__"))
                        conn.commit()
                        cur.close()
                ...
                try:
                    with conn.cursor() as cur:
                        ...
                        if cur.rowcount == 1:
                            ...
                            cur.execute('UPDATE `kikbot_targeting` SET `pending` = 0, `requested` = NOW() WHERE `id` = %s;', (row['id'],))
            ...
            # -=-=-=-=-=-=-=-=- TEXT MESSAGE -=-=-=-=-=-=-=-=-
                ...
                if message.mention is not None:
                    if message.body == "Start Chatting":
                        ...
                        try:
                            ...
                            try:
                                cur.execute('SELECT id FROM mentions WHERE from_user = ? AND participant = ?;', (message.from_user, message.participants[-1]))
                    ...
                    try:
                        with conn.cursor() as cur:
                            cur = conn.cursor()
                            cur.execute('INSERT INTO `kikbot_logs` (`username`, `chat_id`, `body`) VALUES (%s, %s, %s);', (message.from_user, message.chat_id, message.body.encode('utf-8')))
                            conn.commit()
                            cur.close()
                ...
                # -=-=-=-=-=-=-=-=-=- CANCEL TRADE URL -=-=-=-=-=-=-=-
                if message.body.lower() == "cancel" and message.chat_id in item_flips and item_flips[message.chat_id]['trade_url'] == "_{PENDING}_":
                    item_flips[message.chat_id]['trade_url'] = None
                    default_text_reply(message=message)
                ...
                if message.chat_id in item_flips and item_flips[message.chat_id]['trade_url'] == "_{PENDING}_":
                    if re.search(r'^[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)$', message.body) is not None:
                        item_flips[message.chat_id]['trade_url'] = None

                        conn = pymysql.connect(host=Const.DB_HOST, user=Const.DB_USER, password=Const.DB_PASS, db=Const.DB_NAME, charset='utf8mb4', cursorclass=pymysql.cursors.DictCursor);
                        try:
                            with conn.cursor() as cur:
                                cur.execute('UPDATE `item_winners` SET `trade_url` = %s WHERE `id` = %s LIMIT 1;', (message.body, item_flips[message.chat_id]['claim_id']))
                                conn.commit()
                        ...
                        payload = {
                            ...
                            'username'   : "game.bots",
                            'icon_url'   : "http://icons.iconarchive.com/icons/chrisbanks2/cold-fusion-hd/128/kik-Messenger-icon.png",
                            'text'       : "Trade URL for *{from_user}*:\n{trade_url}".format(from_user=message.from_user, trade_url=message.body)
                        }
                        response = requests.post("https://hooks.slack.com/services/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", data={'payload': json.dumps(payload)})
                        ...
                        try:
                            kik.send_messages([
                                TextMessage(
                                    ...
                                    type_time = 150,
                                    keyboards = cancel_keyboard(True)
                                )
                            ])
                        except KikError as err:
                            print("::::::[kik.send_messages] kik.KikError - {message}".format(message=err))
                ...
                # -=-=-=-=-=-=-=-=-=- END SESSION -=-=-=-=-=-=-=-
                if message.body.lower() == "end" or message.body.lower() == "quit":
                    print("-=- ENDING HELP -=-")
                    ...
                    if message.chat_id in game_convos:
                        ...
                        try:
                            conn = sqlite3.connect("{script_path}/data/sqlite3/kikbot.db".format(script_path=os.path.dirname(os.path.abspath(__file__))))
                            cur = conn.cursor()
                            cur.execute('UPDATE targeting SET pending = 0, in_session = 0 WHERE username_id = ? OR participant_id = ?;', (message.chat_id, message.chat_id))
                            conn.commit()
                ...
                if message.body in topic_names:
                    ...
                    try:
                        with conn.cursor() as cur:
                            ...
                            conn.commit()
                            cur.execute('SELECT @@IDENTITY FROM `kikbot_sessions`;')
                            help_convos[message.chat_id]['session_id'] = cur.fetchone()['id']
                            cur.close()
                ...
                if message.body == "Play Flip Coin" or message.body == "Next Item":
                    ...
                    if button_taps[message.chat_id] <= 3000000000:
                ...
                # -=-=-=-=-=-=-=-=- STEAM BTN -=-=-=-=-=-=-=-=-
                if message.body == "Steam" or message.body == "Connect Steam to Trade":
                    modd.utils.send_evt_tracker(category="steam-button", action=message.chat_id, label=message.from_user)
                ...
                if message.body == "Chat & Trade":
                    try:
                        conn = sqlite3.connect("{script_path}/data/sqlite3/kikbot.db".format(script_path=os.path.dirname(os.path.abspath(__file__))))
                        cur = conn.cursor()
                        cur.execute('SELECT id, username, username_id, game_name, game_image FROM targeting WHERE participant = ? AND respond = 1 ORDER BY added DESC LIMIT 1;', (message.from_user,))

                        row = cur.fetchone()
                        if row is not None:
                            try:
                                cur.execute('UPDATE targeting SET pending = 0, in_session = 1 WHERE id = ? LIMIT 1;', (row[0],))
                                conn.commit()
        ...
        if data['token'] == Const.NOTIFY_TOKEN:
            ...
            try:
                ...
                for message in data['messages']:
                    ...
                    try:
                        cur.execute('UPDATE targeting SET respond = 1 WHERE id = ? LIMIT 1;', (message['target_id'],))
